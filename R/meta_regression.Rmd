...
Code adapted from Bonapersona, V. (2019, May 20). The behavioral phenotype of early life adversity: a 3-level meta-analysis of rodent studies. Retrieved from osf.io/ra947
...
Contain Exploratory estimates of trauma and fear memory in clinical data (categories with <4 studies)
```{r}
...
TRACE_clinical_valence_phase <- function(model){
 ...
 extinction <- anova(model, L = c(1,0, 0, 0,0))
  ...
  resultMain[,2] <- round(c(
    neutralLearning$Xb, neutralMemory$Xb,
    fearLearning$Xb, emotionalMemory$Xb,
    extinction$Xb,
        valence.learning$Xb, valence.memory$Xb
...
ForestPlot is not informative, to many papers
...
Forestplot not informative (too many papers for visability)
...
TRACE_preclinical_valence_phase <- function(model){
  ...
  resultMain[,2] <- round(c(
    ...
    neutralLearning$Xb, neutralMemory$Xb,
    fearLearning$Xb, fearMemory$Xb,  fearExtinction$Xb,
    traumaMemory$Xb, traumaExtinction$Xb,
        valence.learning$Xb, 
    valence.memory.NF$Xb, valence.memory.NT$Xb, valence.memory.FT$Xb,
    valence.extinction$Xb
...
##### trim and fill
only available for model without moderators, since moderation is present (with opposite effects).. not valid test for robustness of the effects?
```{r eval=FALSE, include=FALSE}
