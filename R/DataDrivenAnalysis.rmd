...
```{r}
jpeg(file="results/metaCART.clinical.lookaheadT.jpeg", height =3000, width = 2000, res=300, pointsize = 12)
plot(REtree.C)
dev.off()
```
...
```{r}
jpeg(file="results/metaCART.preclinical.lookaheadT.jpeg", height = 3000, width = 2000, res=300, pointsize = 12)
plot(REtree.P)
dev.off()
```
...
{jpeg(file="results/metaforest_PD_clinical.jpeg", height = 1500, width = 4000, res=300) # Higher quality output
...
```{r}
PartialDependence(cv.mf.cluster.clinical$finalModel, vars="age.PTSD.cat", moderator = "subject")
```


```{r}
PartialDependence(cv.mf.cluster.clinical$finalModel, vars="age.PTSD.cat", moderator = "type")
...
imp.preclinical[order(imp.preclinical$overall,decreasing = T),] %>%
   filter(overall >0.5) -> important.vars.preclinical
   # slice_head(.,n=nrow(.) / 2) 
...
{jpeg(file="results/metaforest_PD_preclinical.jpeg", height = 1500, width = 4000, res=300) # Higher quality output
