...
arrangeGrob(grobs=grobz.clinical, 
            ...
            # respect = F,
            ncol=3, #nrow=4, 
            left="Hedge'G", bottom = "Variable levels")->PD.clinical # https://community.rstudio.com/t/common-axis-title-in-grid-arrange/96353/2
grid.newpage()
...
### Model convergence
```{r}
tiff(file="results/metaforest_Preclinical_convergence.tiff", height=800, width=800, pointsize=8,
     res=300)
...
Create PD list
```{r}
PartialDependence(cv.mf.cluster.preclinical$finalModel, vars = variable.preclinical, bw=T, rawdata=F, #pi = .95
                   output = 'list'
                  ) ->PD.list.preclinical
```
...
arrangeGrob(grobs=grobz.preclinical, 
            ncol=3, #nrow=4, 
            left="Hedge'G", bottom = "Variable levels")->PD.preclinical # https://community.rstudio.com/t/common-axis-title-in-grid-arrange/96353/2
grid.newpage()
grid.draw(PD.preclinical)
}


 {jpeg(file="results/metaforest_PD_preclinical.jpeg", height = 3000, width = 2000, res=300) # Higher quality output
grid.draw(PD.preclinical)
 dev.off()}
```
...
# tiff(file="results/VarImp.clinical.preclinical2.tiff", height=800, width=800, pointsize=8, res=300)
# var.imp.plots
#   dev.off()
```
