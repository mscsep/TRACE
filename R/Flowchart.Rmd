...
# some tryout code
 ...
 > some question marks in inclusion.final S2 en missing values in S3?
...
data.s3.animal %>% filter(!(inclusion.final %in% c("0","1"))) %>% select(`MetaData_FINAL incl/excl_comment`)
data.s3.human %>% filter(!(inclusion.final %in% c("0","1"))) %>% select(`MetaData_FINAL incl/excl_comment`)

data.s3.animal$inclusion.final
data.s3.human$inclusion.final
```
