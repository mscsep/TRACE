...
search1_animal <- search1_animal_uPMID[1:(length(search1_animal_uPMID)-2)]# remove last 2 items in vector (these are not PMIDs, but counting/notes)
...
search1_human <- search1_human_uPMID[1:(length(search1_human_uPMID)-2)]# remove last 2 items in vector (these are not PMIDs, but counting/notes)
...
## remove local copies of downloaded OSF files
```{r }
file.remove(c("data/pubmed_result_search2_animal 6.1.20.txt","data/pubmed_result_search2_human 6.1.20.txt", "data/pmid.animal.s3.learn.22.5.20.txt", "data/pmid.human.s3.learn.22.5.20.txt"))
```
...
unique(screened_articles) ->unique_screened_articles
...
## remove local copies of downloaded OSF files
```{r}
file.remove(c("data/TRACE data_collection_search1.xlsx"))
```
...
# remove local copies of downloaded OSF files
 file.remove(c(
   "data/required_full_checks_animal_s2_MS.csv","data/required_full_checks_human_s2_SH.csv"#,
...
s3.A.n.post.screening # NB: 126 without "?"
```
...
### remove local copies of downloaded OSF files
```{r}
file.remove(c("data/Checked_TRACE_screening_search3_MS_EG.xlsx", "data/TRACE_data_collection_search3.xlsx"))
```
...
```{r}
readRDS("processed_data/TRACEprepared.RData")->dataset # This is the output of `prepare_effect_size_QA.rmd`, see README file
...
dataset %>% 
  distinct(new.idHC, .keep_all = TRUE) %>% 
   # group_by(subject, sex.PTSD) %>%
     group_by(subject.cat) %>%
  # group_by(subject) %>%  
  tally(round(nHC,0)) ->unique.n.HC.subjects
...
#### PTSD
```{r}
# load prepared data for descriptive (export created in analysis scripts theoretical and data-driven analysis)
readRDS("processed_data/clinical.data.metaregression.RDS") -> metaregression.clinical 
...
(g13 <- boxGrob(paste0(
  # "in meta-analysis:\n- ",# mReg.clinical.ptsd$papers, ":\n- ",
  #                      mReg.clinical.ptsd$groups, " independent patient groups\n - ",
  #                      mReg.clinical.ptsd$n, " unique patients\n - ",
  #                      mReg.clinical.hc$groups, " independent control groups\n - ",
  #                      mReg.clinical.hc$n, " unique controls"
  # 
...
# (g23 <- boxGrob(paste0(
#   # "independent PTSD groups \nin meta-analysis N = ",mReg.preclinical.ptsd$groups
#     "in meta-analysis:\n- ",# mReg.preclinical.ptsd$papers, ":\n- ",
#                        mReg.preclinical.ptsd$groups, " independent patient groups\n - ",
#                        mReg.preclinical.ptsd$n, " unique patients\n - ",
#                        mReg.preclinical.hc$groups, " independent control groups\n - ",
#                        mReg.preclinical.hc$n, " unique controls "
#   ),
