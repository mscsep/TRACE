...
- valence # NB codes: # T=trauma; N=non_trauma_neutral; E=non_trauma_emotional; F=non-trauma_fearful
...
- no emotional papers in animal

> New valences grouping created: stressful vs non-stressful learning/memory
...
> NB Trauma could be a special form of stressful learning, Consider exclusion of T from 'stresfful' in 'analysis script'


## Comparison x valence as moderators?
```{r}
# df10 %>% filter(is.na(valence)) %>% select(PMID, task.d, measure.d, task.m, measure.m) %>% head()
df10 %>% group_by(subject.cat, Valence_Grouped, comparison) %>%
...
- no stress context studies in human
...
df10 %>% 
  ...
  summarize(papers=length(unique(PMID)), comparisons=length(each))
```
also possible (behalve 'manipulation in animals', just one paper)
