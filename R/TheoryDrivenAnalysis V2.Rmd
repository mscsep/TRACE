...
```{r}
# # potential outliers
# clinical.inf[which(clinical.inf$potOut == 1),] %>% nrow()
...
```{r}
data.frame(cbind(mod.A$b, CIlb=mod.A$ci.lb, CIub=mod.A$ci.ub, p=mod.A$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.A.csv")
```
...
```{r}
data.frame(cbind(mod.B$b, CIlb=mod.B$ci.lb, CIub=mod.B$ci.ub, p=mod.B$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.B.csv")
```
...
```{r}
data.frame(cbind(mod.C$b, CIlb=mod.C$ci.lb, CIub=mod.C$ci.ub, p=mod.C$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.C.csv")
```
...
funnel(mod.preclinical, legend = F, col=preclinical$PMID, back = 'white'#,xlab='mean DR'
)
dev.off()
```
...
```{r}
data.frame(cbind(mod.E$b, CIlb=mod.E$ci.lb, CIub=mod.E$ci.ub, p=mod.E$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.E.csv")
...
```{r}
data.frame(cbind(mod.D$b, CIlb=mod.D$ci.lb, CIub=mod.D$ci.ub, p=mod.D$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.D.csv")
```
...
```{r}
data.frame(cbind(mod.F$b, CIlb=mod.F$ci.lb, CIub=mod.F$ci.ub, p=mod.F$pval)) %>% 
  round(.,4) %>% 
  # filter(p <=0.05) %>% 
  write.csv2(.,"results/sens.F.csv")
```
