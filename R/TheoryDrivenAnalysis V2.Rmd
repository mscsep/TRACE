...
```{r}
fsn.table.clincal <- data.frame(matrix(data = NA, nrow = 5, ncol = 3)) #V new

names(fsn.table.clincal)<-c("valence", "phase", "Fail-save N")
...
fsn.table.clincal[1,] <- c("F", "E", fsnFE.clinical$fsnum)
...
fsn.table.clincal[2,] <- c("F", "L", fsnFL.clinical$fsnum)
...
fsn.table.clincal[3,] <- c("N", "L", fsnNL.clinical$fsnum)
...
fsn.table.clincal[4,] <- c("E", "M", fsnEM.clinical$fsnum)
...
fsn.table.clincal[5,] <- c("N", "M", fsnNM.clinical$fsnum)
...
```{r}
 rma.mv(yi, vi,
        ...
        data = clinical.filtered.HC.PTSD) %>% summary()
```
 
##### non-trauma exposed vs trauma-exposed HC
...
rma.mv(yi, vi,
       ...
       data = clinical.filtered.TC.PTSD) %>% summary()
...
mod.A.bias <- rma.uni(yi, vi,
                      ...
                      slab = preclinical$reference, 
                      data = preclinical) 
funnel(mod.A.bias) # same shape as above
```
...
misschien beter op subsets om moderatie te respecteren?

valenceF:phaseE   -0.9416  0.1345  -6.9992  <.0001  -1.2053  -0.6779  *** 
valenceT:phaseE   -1.4740  0.2114  -6.9725  <.0001  -1.8884  -1.0597  *** 
valenceF:phaseL    0.4790  0.1812   2.6441  0.0082   0.1239   0.8340   ** 
valenceN:phaseL   -1.1640  0.1533  -7.5922  <.0001  -1.4645  -0.8635  *** 
valenceF:phaseM    1.2359  0.1537   8.0384  <.0001   0.9345   1.5372  *** 
valenceN:phaseM   -1.3116  0.1735  -7.5595  <.0001  -1.6517  -0.9716  *** 
valenceT:phaseM    2.0467  0.1390  14.7285  <.0001   1.7743   2.3190  *** 


```{r}
fsn.table <- data.frame(matrix(data = NA, nrow = 7, ncol = 3)) #V new

names(fsn.table)<-c("valence", "phase", "Fail-save N")

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "F" & preclinical$phase == "E"),
    type = "Rosenthal") ->fsnFE

fsn.table[1,] <- c("F", "E", fsnFE$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "T" & preclinical$phase == "E"),
    type = "Rosenthal")->fsnTE

fsn.table[2,] <- c("T", "E", fsnTE$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "F" & preclinical$phase == "L"),
    type = "Rosenthal") ->fsnFL

fsn.table[3,] <- c("F", "L", fsnFL$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "N" & preclinical$phase == "L"),
    type = "Rosenthal")->fsnNL

fsn.table[4,] <- c("N", "L", fsnNL$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "F" & preclinical$phase == "M"),
    type = "Rosenthal")->fsnFM

fsn.table[5,] <- c("F", "M", fsnFM$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "N" & preclinical$phase == "M"),
    type = "Rosenthal")->fsnNM

fsn.table[6,] <- c("N", "M", fsnNM$fsnum)

fsn(yi = yi, vi = vi, 
    data = preclinical, 
    subset = (preclinical$valence == "T" & preclinical$phase == "M"),
    type = "Rosenthal")->fsnTM

fsn.table[7,] <- c("T", "M", fsnTM$fsnum)


fsn.table

write.csv2(fsn.table, "results/phase_valence_PTSD_preclinical.FSN.csv")
```
...
# funnel(mod.A.bias) # same shape

# summary(mod.A.bias)
trimfill(mod.A.bias2) # only available for model with moderators  -> maar eigenlijk vraag ik me af of deze analyse dan wel valid is.. want de moderatie effecten zitten WEL in de data.. (en zijn tegenovergesteld voor sommige levels...)
```
...
data = preclinical.HC.PTSD) %>% summary()
```
...
data = preclinical.HC.TC) %>% summary()
```
...
rma.mv(yi, vi,
       ...
       data = preclinical.TC.PTSD) %>% summary()
```
