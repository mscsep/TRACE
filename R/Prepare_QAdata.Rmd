...
ggplot(QA.H.recoded.long, aes(fill=score, x=n, y=question)) + 
  ...
  labs(x="", y="", title="Study Quality: Bias assessment", #tag = "C)", 
       subtitle = "animal studies",
       legend.title="test")+
  scale_fill_brewer(palette = "Blues", direction = 1)+

theme_minimal() +
     theme(legend.title = element_blank())->QA.clincal
QA.clincal
```

```{r}
ggsave("results/RoB_clinical.jpeg",plot=QA.clincal,width = 15, height = 10, units = "cm")
```
...
count(question, score) %>%
    mutate(
     #  question = factor(question, levels = (unique(question))), # reorder levels
# question = case_when(
#   grepl("seqGeneration", question) ~ "Was the allocation sequence adequately generated and applied?",
#   grepl("baseline", question) ~ "Were the groups similar at baseline or were they adjusted for confounders in the analysis?",
#     grepl("allocation", question) ~ "Was the allocation adequately concealed?",
#      grepl("housing", question) ~ "Were the animals randomly housed during the experiment?",
#      grepl("blindExp", question) ~ "Were the caregivers and/or investigators blinded from knowledge which intervention each animal received during the experiment?",
#      grepl("outAss", question) ~ "Were animals selected at random for outcome assessment?",
#      grepl("outBlind", question) ~ "Was the outcome assessor blinded?",
#      grepl("incData", question) ~ "Were incomplete outcome data adequately addressed?"
# 
# ),
...
# question=reorder(question, desc(question)),
         score=factor(score, levels = c("0","0.5","1"),  # Y=0=low risk, N=1=high risk
                      labels=c("Low RoB", "Unclear", "High RoB"))
       ) -> QA.A.recoded.long
...
#   labels = c(
#   unique(grep("seqGeneration", question,value = T)) = "Was the allocation sequence adequately generated and applied?",
#   unique(grep("baseline", question,value = T)) = "Were the groups similar at baseline or were they adjusted for confounders in the analysis?",
#    unique( grep("allocation", question,value = T)) = "Was the allocation adequately concealed?",
#     unique( grep("housing", question,value = T)) = "Were the animals randomly housed during the experiment?",
#     unique( grep("blindExp", question,value = T)) = "Were the caregivers and/or investigators blinded from knowledge which intervention each animal received during the experiment?",
#     unique(grep("outAss", question,value = T) )= "Were animals selected at random for outcome assessment?",
#      unique(grep("outBlind", question,value = T)) = "Was the outcome assessor blinded?",
#     unique( grep("incData", question,value = T)) = "Were incomplete outcome data adequately addressed?")) +
